    ...
    class WC_Gateway_Bunq_Ideal extends WC_Payment_Gateway {
 	    ...
 	    */
 		public function __construct() {
			...
			$this->bunqme_name  = $this->get_option('bunqme_name');
            
            $this->enabled =  $this->get_option('enabled');
            
            
            add_action('woocommerce_update_options_payment_gateways_' . $this->id, array(&$this, 'process_admin_options'));
			add_action('woocommerce_api_' . strtolower(get_class($this)), array( &$this, 'check_ipn' ));

			// Valid for use.
			

			// Checking if apikey is not empty.
			if (empty($this->bunqme_name) === true) {
				add_action('admin_notices', function(){
                    ?>
                    <div class="notice notice-error">
                        <p><?php _e( 'your bunq.me username is not specified. The payment gateway will not work until this is fixed', 'bunq-ideal' );?></p>
                    </div>
                    <?php
                } );
			}
    ...
    <?php
        ...
        */
        ...
        public function generate_ideal_link($amount, $desc, $issuer){
			...
			return generateIdealUrl($this->bunqme_name, $amount, $desc, $issuer)['url'];
